...
class LoanDataLabelPredictor(BaseEstimator, TransformerMixin):
    def __init__(self, include_post_origin=[]):
        self.include_post_origin = include_post_origin
    ...
    def transform(self, X, y=None):
        ...
        return (
            X[self.applicant_features_ + self.loan_features_ + self.include_post_origin],
            label
            )
...
class LoanDataMissingHandler(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        if y:
            return self.X.reset_index(drop=True), y[self.X.index]
        return self.X.reset_index(drop=True)